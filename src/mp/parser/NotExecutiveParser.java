  ...
  private void CreateUsedVariablesList() throws ScriptException{
    ...
    while ( currentParserVar != null ){
      if ( !ScriptLanguageDef.IsServiceName(currentParserVar.GetName()) ) {
        ...
        if ( selfVar == null ){
          ScriptException e = new ScriptException("В собственном списке переменных отсутствует переменная \""
                  + currentParserVar.GetName() + "\"");
          throw e;
        }
        if ( !selfVar.GetTypeName().equalsIgnoreCase( currentParserVar.GetTypeName() ) ){
          ScriptException e = new ScriptException("Не совпадают типы у переменных \""
