...
public class Model extends ModelEventGenerator implements Runnable, ModelExecutionManager{
 ...
 /**Изменение текущего модельного времени. Изменение производится с учетом ближайшего события в системе и шага
  * моделирования.
  * @param aNearestEventTime
  */
  private void UpdateCurrentTime( ModelTime aNearestEventTime ){
   /**Если в системе нет ближайшего события, то модельное время увеличивается на шаг моделирования
    */
    if ( aNearestEventTime == null ){
      FCurrentModelTime.Add( FTimeIncrement );
      return;
    }
   /**В модели присутствует ближайшее событие. Необходимо выяснить, что произойдет раньше - следующий запланированный
    * такт, или событие. Если плановый такт, то это означает, что в модели до этого такта ничего не происходит,
    * и делать этот такт не нужно для экономии процессорного времени. В этом случае следующий такт нужно производить
    * либо во время этого события, либо ПОСЛЕ него.
    * Если же плановый такт должен произойти после ближайшего события, то модельное время становится равным времени
